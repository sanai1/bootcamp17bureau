...
async def translate(user_prompt: str, language: str) -> str:
    system_prompt = f"Необходимо перевезти пользовательский текст на язык {language}. Ответить необходимо ТОЛЬКО на {language}. Не используй ничего кроме {language} языка"
    body = {
